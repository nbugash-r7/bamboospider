...
id="confirmDelete"]
    ...
    [#if buildTaskDefinitionsMap?keys?size > 0]
        ...
        [#list buildTaskDefinitionsMap?keys as t]
            [@s.url id='editBuildTasksUrl' action="editBuildTasks" namespace="/build/admin/edit" buildKey=buildTaskDefinitionsMap.get(t)!"" /]
    ...
    <ul>
        [#list deploymentTaskDefinitionsMap?keys as t]
            [@s.url id='editEnvTasksUrl' action="configureEnvironmentTasks" namespace="/deploy/config" environmentId=deploymentTaskDefinitionsMap.get(t)!"" /]
