...
[#-- @ftlvariable name="error" type="com.atlassian.bamboo.logger.DecoratedErrorDetailsImpl" --]
    ...
    [#if error.throwableDetails??]
        <div class="footer">
            ...
            /]
            <a href="${viewErrorUrl}" rel="help" data-use-help-popup="true">[@ww.text name="plan.error.view"/]</a>
            [@s.url id='removeErrorFromLogUrl' namespace='/' action='removeErrorFromLog'
                buildKey=error.buildKey
                error=error.errorNumber
                returnUrl=removeErrorReturnUrl
            /]
            [@s.form id='removeErrorHiddenForm_${error.errorNumber}' action='${removeErrorFromLogUrl}' formEndClearer=false cssClass='popupRemoveErrorLogForm']
